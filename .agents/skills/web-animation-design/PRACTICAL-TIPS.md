...
## Smooth Container Size Transitions

### Animate Width/Height Based on Content with useMeasure

When content changes inside a container (e.g. switching between dropdown panels, tabs, or accordion content), animating `width`/`height` directly causes a circular dependency — the container's `overflow: hidden` collapses the content, so there's nothing to measure.

**Solution:** Use `react-use-measure` on an inner div, then animate the outer container's dimensions to match.
...
function AnimatedContainer({ children }: { children: React.ReactNode }) {
  ...
  return (
    <motion.div
      ...
      transition={{ type: "spring", stiffness: 440, damping: 38, mass: 0.72 }}
    >
      <div ref={measureRef} className="w-max min-w-56 p-4">
        {children}
      </div>
    </motion.div>
  );
...
- `w-max` on the inner div prevents it from collapsing to the parent's animated width — it maintains its natural size for measurement.
- `min-w-*` sets a floor so the container doesn't shrink to zero before the first measurement.
- Fall back to `"auto"` when bounds are 0 (before first paint).
- Use a spring transition for organic size changes. Use `duration: 0` when opening from nothing (skip the initial animation).
- `overflow-hidden` on the outer div clips content during the transition.

**Common use cases:** Shared navigation dropdowns with different-sized panels, tab content areas, expanding/collapsing sections, and any container where content swaps but you want a smooth size transition instead of a jump.
