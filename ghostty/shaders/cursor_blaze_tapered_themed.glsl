...
// Theme colors from GTK theme - reduced intensity
const vec4 TRAIL_COLOR = vec4(0.545, 0.643, 0.690, 0.4); // #8ba4b0 with reduced alpha (was 0.85)
const vec4 TRAIL_COLOR_ACCENT = vec4(0.949, 0.925, 0.737, 0.5); // #f2ecbc with reduced alpha (was 1.0)
...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
    ...
    float mod = .010; // Softer edges to match GTK theme's border-radius aesthetic
    
    // Trailblaze with theme colors - more subtle blending
    vec4 trail = mix(TRAIL_COLOR_ACCENT, fragColor, 1. - smoothstep(0., sdfTrail + mod, 0.006));
    trail = mix(TRAIL_COLOR, trail, 1. - smoothstep(0., sdfTrail + mod, 0.004));
    trail = mix(trail, TRAIL_COLOR * 0.85, step(sdfTrail + mod, 0.));
    
    // Cursorblaze with accent color - reduced intensity
    trail = mix(TRAIL_COLOR_ACCENT * 0.6, trail, 1. - smoothstep(0., sdfCurrentCursor + .002, 0.003));
    trail = mix(TRAIL_COLOR, trail, 1. - smoothstep(0., sdfCurrentCursor + .002, 0.002));
