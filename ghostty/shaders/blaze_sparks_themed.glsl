...
// Theme colors from GTK theme
const vec3 BLAZE_COLOR = vec3(0.545, 0.643, 0.690); // #8ba4b0 (accent_color)
const vec3 BLAZE_ACCENT = vec3(0.949, 0.925, 0.737); // #f2ecbc (foreground)

void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    vec3 base_color = BLAZE_COLOR;
    ...
    // Use theme colors with subtle accent highlights
    vec3 rgb = c0 * base_color + c1 * base_color * blue_shift;
    
    // Add subtle warm accent tint to the brightest parts
    float brightness = length(rgb);
    rgb = mix(rgb, BLAZE_ACCENT, smoothstep(0.8, 1.5, brightness) * 0.3);
    
    // Add noise
    rgb += hash33(vec3(fragCoord, iTime * 256.)) / 512.;
    
    // Softer intensity for theme consistency
    float mask = clamp(c0 * 0.15, 0.0, 1.0) * fade; // Reduced from 0.2 to 0.15
    
    // Additive blending
    fragColor = fragColor + vec4(rgb * mask, 1.0);
